...
void AChessBoard::BeginPlay()
{
	...
	// TODO : 처음 들어갔을때는 실행x
	ResultUI = Cast<UResultUI>(CreateWidget<UResultUI>(GetWorld(), ResultUIClass));
	ResultUI->AddToViewport();
	ResultUI->ShowResult(EPieceType::King, EPieceColor::Black, EPieceType::King, EPieceColor::White, EPieceColor::White);
}
...
FName AChessBoard::ShowQueenWidget()
{
	FName Selected;
	if (Controller)
	{
		QueenSelectUI = CreateWidget<UQueenSelectWidget>(GetWorld(), QueenSelectUIClass);
		if (QueenSelectUI) // PauseMenuClass는 UUserWidget 서브클래스
		{
